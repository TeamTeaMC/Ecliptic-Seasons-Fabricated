...
public class NotHolderSet<T> implements ICustomHolderSet<T> {
    ...
    public NotHolderSet(HolderGetter<T> registryLookup, HolderSet<T> value) {
        this.registryLookup = (HolderLookup.RegistryLookup<T>) registryLookup;
        this.value = value;
    ...
    public static class Type implements HolderSetType {
        ...
        public <T> MapCodec<? extends ICustomHolderSet<T>> makeCodec(ResourceKey<? extends Registry<T>> registryKey, Codec<Holder<T>> holderCodec, boolean forceList) {
            return RecordCodecBuilder.<NotHolderSet<T>>mapCodec(
                    builder -> builder
                            .group(
                                    RegistryOps.retrieveGetter(registryKey),
                                    ExtendHolderSetCodec.create(registryKey, holderCodec, forceList).fieldOf("value").forGetter(NotHolderSet::value))
